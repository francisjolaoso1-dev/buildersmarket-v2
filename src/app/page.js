...
const [sortOrder, setSortOrder] = useState('default'); // New Sort memory
...
// Processing the New Sorting Feature
...
<button onClick={() => setActiveTab('market')} style={{ background: 'none', border: 'none', color: activeTab === 'market' ?
'#16a34a' : '#4b5563', fontWeight: '600', cursor: 'pointer' }}>Marketplace</button>
<button onClick={() => setActiveTab('supplier')} style={{ background: 'none', border: 'none', color: activeTab === 'supplier' ?
'#16a34a' : '#4b5563', fontWeight: '600', cursor: 'pointer' }}> 🇨🇳  Factory Portal</button>

<div style={{ border: '1px solid #d1d5db', borderRadius: '20px', padding: '3px', display: 'flex', backgroundColor: '#f3f4f6', marginRight: '5px' }}>
<button onClick={() => setViewMode('NGN')} style={{ border: 'none', padding: '5px 12px', borderRadius: '16px', cursor: 'pointer', fontWeight: '700', fontSize: '12px', backgroundColor: viewMode === 'NGN' ?
'#111827' : 'transparent', color: viewMode === 'NGN' ? '#ffffff' : '#4b5563' }}>₦ NGN</button>
<button onClick={() => setViewMode('USD')} style={{ border: 'none', padding: '5px 12px', borderRadius: '16px', cursor: 'pointer', fontWeight: '700', fontSize: '12px', backgroundColor: viewMode === 'USD' ?
'#111827' : 'transparent', color: viewMode === 'USD' ? '#ffffff' : '#4b5563' }}>$ USD</button>
</div>
{currentUser ? (
<div style={{ display: 'flex', alignItems: 'center', gap: '12px', borderLeft: '1px solid #e5e7eb', paddingLeft: '15px' }}>
<span style={{ fontSize: '13px', fontWeight: '600' }}>
👋  {currentUser.name} <span style={{ fontSize: '10px', backgroundColor: currentUser.accountType === 'corporate' ? '#dbffe4' : '#e5e7eb', color: currentUser.accountType === 'corporate' ?
'#15803d' : '#111827', padding: '2px 6px', borderRadius: '10px', marginLeft: '4px', fontWeight: '800' }}>{(currentUser.accountType || 'individual').toUpperCase()}</span>
</span>
...
{/* SEGMENT 5: MODAL OVERLAYS (AUTH & TRACKING) */}
{showAuthModal && (
...
<button type="button" onClick={() => setProcurementMode('individual')} style={{ flex: 1, padding: '8px', borderRadius: '6px', border: 'none', fontSize: '13px', fontWeight: '700', cursor: 'pointer', backgroundColor: procurementMode === 'individual' ?
'#ffffff' : 'transparent', color: procurementMode === 'individual' ? '#111827' : '#6b7280' }}> 👤  Individual Client</button>
<button type="button" onClick={() => setProcurementMode('corporate')} style={{ flex: 1, padding: '8px', borderRadius: '6px', border: 'none', fontSize: '13px', fontWeight: '700', cursor: 'pointer', backgroundColor: procurementMode === 'corporate' ?
'#ffffff' : 'transparent', color: procurementMode === 'corporate' ? '#111827' : '#6b7280' }}> 🏢  Corporate / Firm</button>
</div>
...
<div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Customs Duty Premium:</span><strong>{isDesign ?
'EXEMPT (Digital Asset)' : (viewMode === 'NGN' ? modalItem.dutyNgn : modalItem.dutyUsd)}</strong></div>
...
Account Assignment: <strong style={{ color: '#111827' }}>{procurementMode === 'corporate' ? corpDetails.companyName ||
'Corporate Account' : (currentUser ? currentUser.name : 'Individual Builder')}</strong>
</p>
...
{trackingStep === 4 && <div> ✅  <strong>Stage 4: Ready for Delivery</strong><p style={{ margin: '4px 0 0', color: '#16a34a', fontSize: '12px' }}>Cargo arrived at local distribution node.
Ready for site dispatch.</p></div>}
</div>
) : (
<div>
...
<button onClick={() => setTrackingStep(prev => Math.min(prev + 1, 4))} disabled={trackingStep === 4} style={{ flex: 1, backgroundColor: '#111827', color: '#fff', border: 'none', padding: '10px', borderRadius: '6px', cursor: 'pointer' }}>{trackingStep === 4 ?
'Order Complete' : 'Advance Stage'}</button>
