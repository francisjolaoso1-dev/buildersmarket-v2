...
export default function Home() {
  ...
  const handleOnboardAsset = (e) => {
    ...
    const calculatedDutyNgn = isDesign ? 0 : Math.round(baseNgnNum * 0.07);
    const totalNgnNum = baseNgnNum + calculatedDutyNgn;
    const baseUsdNum = Math.round((baseNgnNum / 1600) * 100) / 100;
    const calculatedDutyUsd = isDesign ? 0 : Math.round((calculatedDutyNgn / 1600) * 100) / 100;
                ...
                <div key={product.id} style={{ backgroundColor: '#ffffff', borderRadius: '12px', border: '1px solid #e5e7eb', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                  ...
                  <div style={{ padding: '0 20px 20px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid #f3f4f6', paddingTop: '15px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      <button onClick={() => { setModalItem(product); setInvoiceConfirmed(false); setTrackingStep(1); setProcurementMode(currentUser ? currentUser.accountType : 'individual'); setShowModal(true); }} style={{ backgroundColor: '#16a34a', color: '#fff', border: 'none', padding: '9px 14px', borderRadius: '6px', fontWeight: '600', fontSize: '13px', cursor: 'pointer' }}>Request Invoice</button>
                      <button onClick={() => deleteProduct(product.id)} style={{ backgroundColor: '#fee2e2', color: '#ef4444', border: '1px solid #fca5a5', padding: '5px 14px', borderRadius: '6px', fontWeight: '600', fontSize: '11px', cursor: 'pointer' }}>🗑 ️ Delete</button>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {showAuthModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(11,24,39,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100 }}>
          <div style={{ backgroundColor: '#ffffff', padding: '30px', borderRadius: '12px', maxWidth: '400px', width: '100%', margin: '20px', position: 'relative' }}>
            ...
            <p style={{ color: '#6b7280', fontSize: '13px', marginBottom: '20px', textAlign: 'center' }}>{authView === 'login' ? 'Enter credentials to manage active trade files.' : 'Select builder or firm account level.'}</p>
            <form onSubmit={handleAuthSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <button type="submit" style={{ width: '100%', backgroundColor: '#16a34a', color: '#fff', border: 'none', padding: '12px', borderRadius: '6px', fontWeight: '700', fontSize: '14px', cursor: 'pointer', marginTop: '5px' }}>{authView === 'login' ? 'Authenticate Entry' : 'Register Profile Matrix'}</button>
            </form>
            <div style={{ textAlign: 'center', marginTop: '15px', fontSize: '12px', color: '#4b5563' }}>{authView === 'login' ? <span>New builder? <button onClick={() => setAuthView('register')} style={{ background: 'none', border: 'none', color: '#16a34a', fontWeight: '700', cursor: 'pointer', padding: 0 }}>Create account</button></span> : <span>Have an account? <button onClick={() => setAuthView('login')} style={{ background: 'none', border: 'none', color: '#16a34a', fontWeight: '700', cursor: 'pointer', padding: 0 }}>Sign in</button></span>}</div>
          </div>
        </div>
      )}
      {showModal && modalItem && (
          ...
          <div style={{ backgroundColor: '#ffffff', padding: '25px', borderRadius: '12px', maxWidth: '460px', width: '100%', margin: '20px', position: 'relative', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            {!invoiceConfirmed ? (
              <>
                ...
                <div style={{ backgroundColor: '#f3f4f6', padding: '12px', borderRadius: '6px', marginBottom: '15px', fontSize: '13px' }}><strong>Asset Line:</strong> {modalItem.name} {modalItem.icon}<div style={{ color: '#6b7280', fontSize: '12px', marginTop: '3px' }}>Dispatch Hub: {modalItem.origin}</div></div>
