...
export default function Home() {
  ...
  const handleAuthSubmit = (e) => {
    ...
    if (authView === 'register') {
      ...
      alert('Success! Your account has been registered.');
  ...
  return (
    <div style={{ fontFamily: 'system-ui, sans-serif', backgroundColor: '#f9fafb', minHeight: '100vh', color: '#111827' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '25px' }}>
            {filteredProducts.map((product) => (
              <div key={product.id} style={{ backgroundColor: '#ffffff', borderRadius: '12px', border: '1px solid #e5e7eb', padding: '20px' }}>
                ...
                <p>{viewMode === 'NGN' ? product.ngn : product.usd}</p>
                <button onClick={() => { setModalItem(product); setShowModal(true); }} style={{ backgroundColor: '#16a34a', color: '#fff', padding: '10px', borderRadius: '6px' }}>Request Invoice</button>
              </div>
            ))}
          </div>
        </main>
      ) : (
        <div style={{ textAlign: 'center', padding: '50px' }}>Factory Portal Coming Soon.</div>
      )}

      {/* --- Auth Modal --- */}
      {showAuthModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '12px', width: '300px' }}>
            <h3>{authView === 'login' ? 'Sign In' : 'Register'}</h3>
            <form onSubmit={handleAuthSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <input type="password" placeholder="Password" onChange={(e) => setAuthPassword(e.target.value)} required />
              <button type="submit">{authView === 'login' ? 'Login' : 'Register'}</button>
            </form>
      ...
      {/* --- Invoice Modal & Tracking --- */}
      {showModal && modalItem && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '12px', width: '400px' }}>
            {!invoiceConfirmed ? (
              <>
                <h3>Proforma for {modalItem.name}</h3>
                <button onClick={() => setInvoiceConfirmed(true)} style={{ width: '100%', padding: '12px', backgroundColor: '#16a34a', color: '#fff', border: 'none', borderRadius: '6px' }}>Confirm & Track Order</button>
              </>
            ) : (
              <>
                <h3>Tracking Progress</h3>
                {trackingStep === 1 && <p>🏭 Stage 1: Factory Production</p>}
                {trackingStep === 2 && <p>🚢 Stage 2: Ocean Freight Transit</p>}
                {trackingStep === 3 && <p>🔄 Stage 3: Quality Compliance: Validating site integrity.</p>}
                {trackingStep === 4 && <p>🚀 Stage 4: Digital Delivery: Assets released.</p>}
                <button onClick={() => setTrackingStep(trackingStep < 4 ? trackingStep + 1 : 1)} style={{ width: '100%', padding: '10px', marginTop: '10px' }}>
                  {trackingStep < 4 ? 'Simulate Next Stage' : 'Close Tracker'}
                </button>
              </>
            )}
            <button onClick={() => setShowModal(false)} style={{ width: '100%', marginTop: '10px' }}>Cancel</button>
          </div>
        </div>
      )}
    </div>
